...
struct PlayerView: View {
    ...
    private var contentView: some View {
        VStack(spacing: UIScreen.main.scale < UIScreen.main.nativeScale ? 16 : 20) {
            if let currentTrack = playerEngine.currentTrack {
                ...
                if settings.showAudiophileInfo {
                    AudiophileDashboardView(
                        dacManager: DACOutputManager.shared,
                        formatName: audiophileFormatName(for: currentTrack),
                        bitDepth: currentTrack.bitDepth,
                        sampleRateHz: currentTrack.sampleRate.map(Double.init)
                    )
                }
    ...
    // MARK: - Audiophile Format Helper

    private func audiophileFormatName(for track: Track) -> String {
        ...
        case "m4a": return (track.bitDepth != nil) ? "ALAC" : "AAC"
        case "aac": return "AAC"
        case "mp3": return "MP3"
        case "ogg": return "OGG"
        ...
        case "tak": return "TAK"
        case "dsf", "dff":
            guard let rate = track.sampleRate else { return "DSD" }
            if rate >= 11_000_000 { return "DSD256" }
            if rate >= 5_500_000 { return "DSD128" }
            return "DSD64"
        default:
